...
All these instruments above essentially function the same, but have different throughput capacity depending on the type, e.g., typically either 12, 48 or 96 samples. The cost to running a sample is more than Qubit and NanoDrop, but will be highly variable depending on which type the facility has. It will likely be more cost effective if you can run as many of your samples as possible in one run, or the technician may plan to run your samples at the same time as other people's samples on one multiwell plate. 
...
Lastly, the DV200 value can also be used for FFPE/degraded samples, see [Agilent documentation here.](https://www.agilent.com/cs/library/technicaloverviews/public/to-ffpe-rna-quality-assessment-ba-fa-systems-5994-8051en-agilent.pdf)
:::
...
![](figures/ont-lib-prep-cdna-01.png)

Image from [link here.](https://nanoporetech.com/document/ligation-sequencing-v14-direct-cdna-sequencing)
:::
...
![](figures/ont-lib-prep-rna-03.png)
...
[Image from link here.](https://nanoporetech.com/document/direct-rna-sequencing-sqk-rna004)
...
::: {.callout-note collapse="true"}
# **Extra for the curious:** **The two main kit options for Illumina RNA library prep.** 
There are two main versions of the Illumina RNA library prep kits -- The "Illumina TruSeq" kit and the "Illumina Stranded" kit. Within each of these versions are also several types of kits for specific purposes (e.g,. total RNA or mRNA). Both are very robust options and produce good quality sequence data. TruSeq requires a slightly higher minimum mRNA input (100 ng) vs. the minimum mRNA input for Illumina Stranded (25 ng), so if you have very low amounts of RNA you may need to use the "Illumina Stranded" kit. The "Illumina Stranded" also has the advantages of being a slightly faster protocol to complete, and allows for higher multiplexing (up to 384 samples, vs up to 96 samples for TruSeq).  Note that both kits do strand specific sequencing. For most generic RNAseq projects, it won't matter which one you use. You may note that some NZ sequencing facilities only offer one or the other kit. There are many other brands and suppliers too that are often Illumina-compatible, *e.g.,* Lexogen QuantSeq.      
...
**DNA sequencing:** [whole genome, shotgun or metagenomic; which one is right for you?](https://www.genomics.auckland.ac.nz/services/metagenomic-sequencing/)

**RNA sequencing:** [bulk RNA-seq, single cell RNA-seq, mRNA vs total, ONT RNA-seq; which one is right for you?](https://www.genomics.auckland.ac.nz/services/rna-sequencing/)
...
|--------|----------------|---------------------|----------------------|
| **How sequencing works** | [Sequencing-by-synthesis](https://www.youtube.com/watch?v=EDVKxSNdSic&t=2s) (fluorescent nucleotides added one base at a time) | [Single-molecule real-time (SMRT) sequencing](https://www.youtube.com/watch?v=7yYPHatccgw) (polymerase incorporates fluorescent bases inside Zero-Mode Waveguides (ZMW)) | [Nanopore sensing](https://www.youtube.com/watch?v=RcP85JHLmnI&list=PLxpxXZj-IgXyR0it4QjoBETC5JeEanW-o&index=1) (changes in ionic current as DNA/RNA passes through a pore) |
...
Lastly, if you are doing short-read sequencing (*i.e.,* Illumina), the sequencing facility will ask you *what read length you want*. You can typically choose between 50bp-300bp (platform-dependent). Read length is determined by the number of sequencing cycles (more cycles = more reagents and time = higher cost). The choice between shorter or longer read length will be a balance of cost and the amount of information for each sample that you need (complex, novel or *de novo* transcriptomes require longer read lengths; more straightforward analyses with well-annotated genomes can utilise shorter read lengths). There is also a trade-off between cost and how many samples you can sequence at a given depth -- because longer reads require more sequencing cycles, choosing longer read lengths reduces how many samples you can multiplex to the same depth within a fixed budget. 

In contrast, for long-read sequencing platforms (ONT and PacBio), you do not specify a fixed read length. Instead, reads are generated as single, continuous sequences, and their length is determined by the size of the input molecules, the library preparation method, and the sequencing chemistry.

Choosing a ***read length*** is a ***trade-off between cost, and the amount of information*** you will recover:

**Table 3:** Read length (short-read Illumina RNA-seq)  

*Note: this table is specific to RNA-seq, but short read sequencing is also used for other applications such at DNA-seq, WGS, ChIP-seq etc. The cost-vs-information trade-off still applies, but the specific pros and cons differ.*

| Read length | Pros | Cons |
|-----|----------|----------|
| **50 bp** | Lowest cost; highest sample multiplexing at a given budget; sufficient for basic gene-level DGE in well-annotated genomes | Poor isoform and splice junction resolution; higher multi-mapping |
| **100 bp** | Good balance of cost and information; reliable splice junction detection; widely used for standard RNA-seq | More sequencing cycles and cost than 50 bp; may miss very complex isoforms |
| **150 bp** | Improved isoform resolution; better mapping across repetitive regions; useful for novel transcript discovery | Higher cost per run; at a fixed budget supports fewer samples at the same depth  |
| **300 bp** | Maximum per-read information; helpful for *de novo* transcriptome assembly | Rarely necessary for Illumina RNA-seq; most expensive |
...
**Performs:** Moderate to large scale RNAseq, DNAseq, amplicon sequencing, other sequencing. Illumina sequencing service of any client-prepared Illumina compatible libraries and library prep service considered on case-by-case basis. ONT platform requires the users to undertake the library and sequencing themselves with supported training and trouble-shooting advice.   
